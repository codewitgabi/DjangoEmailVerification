...
def verify_email(request, form):
    ...
    html_content = render_to_string(
        "email_verification/mail.html", {"token": token}, request=request
    )

    mail = EmailMessage(subject, html_content, to=[email])
...
def pre_login(request, token):
    ...
    return render(request, "email_verification/pre-login-failure.html")
